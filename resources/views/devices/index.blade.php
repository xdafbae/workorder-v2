    ...
    <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex gap-2">
                ...
                <button type="button" data-open-unit-modal class="inline-flex items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <label class="block">
                    ...
                    <div class="flex gap-2 mt-1">
                        <select id="deviceUnitInput" name="unit_id" required class="w-full rounded-md border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100">
                            @foreach ($units as $unit)
                                <option value="{{ $unit->id }}" @selected((string) old('unit_id') === (string) $unit->id)>{{ $unit->name }}</option>
                            @endforeach
                        </select>
                        <button type="button" data-open-unit-modal class="inline-flex items-center justify-center rounded-md border border-slate-200 px-3 bg-slate-50 hover:bg-slate-100 text-slate-700 font-semibold cursor-pointer" title="Tambah ruangan baru">
                            <i data-lucide="plus" class="h-4 w-4"></i>
                        </button>
                    </div>
                </label>
...
<div id="unitModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/60 p-4">
    <div class="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg bg-white shadow-xl">
        <div class="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
            <div>
                <p class="text-sm font-semibold uppercase tracking-wide text-cyan-700">Tambah Ruangan</p>
                <h3 class="mt-1 text-xl font-bold text-slate-950">Data ruangan baru</h3>
            </div>
            <button type="button" data-close-unit-modal class="inline-flex h-9 w-9 items-center justify-center rounded-md border border-slate-200 text-slate-600 hover:bg-slate-50" aria-label="Tutup modal">
                <i data-lucide="x" class="h-4 w-4"></i>
            </button>
        </div>

        <form id="unitForm" method="POST" action="{{ route('units.store') }}" class="space-y-4 px-5 py-5">
            @csrf
            <label class="block">
                <span class="text-sm font-semibold text-slate-700">Nama Ruangan</span>
                <input name="name" required placeholder="Nama Ruangan (contoh: ICU Lantai 3)" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100">
            </label>

            <div class="grid gap-4 grid-cols-2">
                <label class="block">
                    <span class="text-sm font-semibold text-slate-700">Lantai</span>
                    <input name="floor" placeholder="Lantai" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100">
                </label>
                <label class="block">
                    <span class="text-sm font-semibold text-slate-700">Gedung</span>
                    <input name="building" placeholder="Gedung" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100">
                </label>
            </div>

            <div class="flex justify-end gap-2 border-t border-slate-200 pt-4">
                <button type="button" data-close-unit-modal class="inline-flex items-center justify-center rounded-md border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                    Batal
                </button>
...
<script>
    ...
    document.querySelectorAll('[data-open-unit-modal]').forEach((button) => {
        button.addEventListener('click', openUnitModal);
    });
    ...
    @if ($errors->any())
        @if ($errors->hasAny(['type', 'serial_number', 'inventory_number', 'unit_id', 'status', 'purchased_at']))
            openDeviceModal();
        @else
            openUnitModal();
        @endif
